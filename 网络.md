...
3XX 重定向  响应结果表明浏览器需要执行某些特殊的处理以正确处理请求。
	301 Moved Permanenty 永久性重定向。该状态码表示请求的资源已被分配了新的 URI，以后应使用资源现在所指的 URL。
...
cache-control: max-age=96247433 （响应头）
expires: Thu, 03 Jan 2019 04:24:16 GMT （请求头）


协商缓存：
1. last-modified —— if-modified-since
If-Modified-Since 比较资源的更新时间 （请求头）
Last-Modified 资源的最后修改日期时间 （响应头）
两者匹配的话会返回304 重定向本地拿缓存

2. etag —— if-none-match
if-none-match: 标识 条件式请求首部 （请求头）
Etag: 资源的匹配信息，标识 （响应头）
