    ...
    methods: {
        ...
        showInfoM(e) {
            ...
            this.map.plugin('AMap.PlaceSearch', () => {
                ...
                placeSearch.searchNearBy('', [lng, lat], 200)
        ...
        onSearchComplete(res) {
            // console.log(res.poiList.pois[0].address)
            console.log(res)
            ...
            for (let i = 0; i < pois.length; i++) {
                ...
                this.marker[i].on('dragend', this.showInfoM)

                /** 点击出现信息窗体
                let _this = this
                _this.marker[i].on('click', e => {
					//构建信息窗体中显示的内容
					var info = [];
					info.push("地址 :测试地址</div></div>");
					_this.infoWindow = new _this.resMap.InfoWindow({
						content: _this.createContent(pois[i]),  //使用默认信息窗体框样式，显示信息内容
						offset: { x: 0, y: -30 },
						autoMove: true
					});
                    // _this.infoWindow.open(_this.map, poi.location)
