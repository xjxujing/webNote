...
|      | ES6                                          | CommonJS                                         |
| ---- | -------------------------------------------- | ------------------------------------------------ |
| 思想 | 静态加载，可以实现`Tree Shaking`来优化代码。 | 动态加载（运行时加载）没有办法在编译时做静态优化 |
|      | 输出的是对外接口（是个只读引用）             | 输出是值的复制拷贝                               |
|      | 有提升效果                                   |                                                  |
...
- CommonJS中`this`指向当前模块，ES6中`this`指向undefined
- CommonJS默认非严格模式，ES6的模块自动采用严格模式

参考：[深入学习CommonJS和ES6模块化规范](https://zhuanlan.zhihu.com/p/346405395)
